...
let double_selectively array indices : unit = List.iter indices ~f:(fun i -> array.(i) <- (array.(i) * 2))
...
let double_matrix matrix : unit = Array.iteri matrix ~f:(fun i row -> (Array.iteri row ~f:(fun j v -> matrix.(i).(j) <- v * 2)))
